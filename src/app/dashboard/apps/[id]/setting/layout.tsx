...
export default function SettingLayout({ params, children }: { params: Promise<{ id: string }>; children: React.ReactNode }) {
  const { id } = use(params);
  ...
  return <div className="flex justify-start">
    <div className="flex flex-col w-60 flex-shrink-0 pt-10 gap-4">
      <Button size="lg"
        ...
        {
          path !== `/dashboard/apps/${id}/setting/storage` ? <Link href={`/dashboard/apps/${id}/setting/storage`}>Storage</Link> : "Api Key"
        }
      </Button>
      <Button size="lg"
        ...
        {
          path !== `/dashboard/apps/${id}/setting/api-Key` ? <Link href={`/dashboard/apps/${id}/setting/api-Key`}>Api Key</Link> : "Api Key"
        }
      </Button>
    </div>
